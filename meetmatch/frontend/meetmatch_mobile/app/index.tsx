...
const BACKEND_LAN_IP = process.env.BACKEND_LAN_IP?.trim();
const EXPLICIT_API_BASE_URL = process.env.EXPO_PUBLIC_API_BASE_URL?.trim();
// Utility: Try LAN_IP first, then BACKEND_LAN_IP if LAN_IP fails
async function fetchWithIpFallback(path: string, options?: RequestInit) {
  const urls = [];
  if (LAN_IP) urls.push(`http://${LAN_IP}:${API_PORT}${path}`);
  if (BACKEND_LAN_IP && BACKEND_LAN_IP !== LAN_IP) urls.push(`http://${BACKEND_LAN_IP}:${API_PORT}${path}`);
  // Always try at least one
  if (urls.length === 0) urls.push(`http://127.0.0.1:${API_PORT}${path}`);
  ...
  for (const url of urls) {
    try {
      ...
      if (!response.ok) {
        // If server responded but with error, don't retry
        return response;
      }
      return response;
    } catch (err) {
  ...
  throw lastError || new Error('All backend IPs failed');
}
...
export default function MeetMatchMobileApp() {
  ...
  const handleSignup = () => {
    ...
    fetchWithIpFallback('/api/signup/', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
        ...signupForm,
